...
class BehaviorBinder(object):
		...
		def newMethod(*argList, **argDict):
			if methodKey in obj.behavior_exposeMethodDict__:
				...
				if len(overrideBehaviorInfoList) > 0:
					_GG("log")("The behavior(id : \"{0}\") would override the method key of \"{1}\"!".format(overrideBehaviorInfoList[0]["behaviorId"], obj._className_));
	...
	def setNewDataByDataKey(self, obj, dataKey):
		...
		if len(dataInfo["behaviorInfos"]) > 0:
			_GG("log")("The behavior(id : \"{0}\") is setting the data key of \"{1}\"!".format(dataInfo["behaviorInfos"][-1]["behaviorId"], obj._className_));
